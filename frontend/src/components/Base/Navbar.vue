    ...
    mounted () {
      ...
      $(function() {
        ...
        setInterval(function() {
          if (window.localStorage.getItem('myblog-token')) {
            // 如果用户已登录，才开始请求 API
            const payload = JSON.parse(atob(window.localStorage.getItem('myblog-token').split('.')[1]))
            ...
            axios.get(path)
              .then((response) => {
                ...
                total_notifications_count = unread_received_comments_count + unread_messages_count + unread_follows_count + unread_posts_likes_count + unread_comments_likes_count + unread_followeds_posts_count
