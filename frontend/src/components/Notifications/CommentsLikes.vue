  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> received Comments Likes <small v-if="likes">(共 {{ likes._meta.total_items }} 条, {{ likes._meta.total_pages }} 页)</small>
        </h3>
...
export default {
  ...
  methods: {
    getUserreceivedCommentsLikes (id) {
      ...
      const path = `/api/users/${id}/received-comments-likes/?page=${page}&per_page=${per_page}`
      this.$axios.get(path)
        .then((response) => {
          // handle success
