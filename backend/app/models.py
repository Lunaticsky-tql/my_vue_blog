...
class User(PaginatedAPIMixin, db.Model):
    ...
    harassers = db.relationship(
        ...
        backref=db.backref("sufferers", lazy="dynamic"),
        lazy="dynamic",
    )
    # 用户注册后，需要先确认邮箱
    ...
    def get_jwt(self, expires_in=3600):
        ...
        payload = {
            ...
            "user_avatar": base64.b64encode(self.avatar(24).encode("utf-8")).decode(
                "utf-8"
            ),
    ...
    def new_received_comments(self):
        ...
        q1 = set(
            Comment.query.filter(
                Comment.post_id.in_(user_posts_ids), Comment.author != self
            ).all()
        )
            ...
            for u in c.likers:
                if u != self:  # 用户自己点赞自己的评论不需要被通知
                    res = db.engine.execute(
                        "select * from comments_likes where user_id={} and comment_id={}".format(
                            u.id, c.id
                        )
                    )
...
db.event.listen(
    Post.body, "set", Post.on_changed_body
    ...
    def to_dict(self):
        data = {
            ...
            "_links": {
                ...
                "parent_url": url_for("api.get_comment", id=self.parent.id)
                if self.parent
                else None,
                "children_url": [
                    url_for("api.get_comment", id=child.id) for child in self.children
                ]
                if self.children
                else None,
            },
